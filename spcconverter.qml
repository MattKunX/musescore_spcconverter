...
MuseScore {
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      Layout.topMargin: 10
      CheckBox {
        id: note_spacesCheckBox
        text: "Spaces between notes"
      }
      CheckBox {
        id: measure_spacesCheckBox
        text: "Spaces between measures"
      }
    }
    ...
    RowLayout {
      ...
      CheckBox {
        id: swing8CheckBox
        text: "Swing 8th notes"
      }
      CheckBox {
        id: swing16CheckBox
        text: "Swing 16th notes"
      }
    }
    ...
    Text {
      Layout.leftMargin: 10
      text: "Header (use for custom instruments, metadata, etc.):"
    }
    TextArea {
      Layout.leftMargin: 10
      text: ""
  ...
  function getSwingDuration(duration, positionInMeasure) {
     ...
     * So, we only need to figure out whether that final eighth
  ...
  function durationStringFromFraction(duration, positionInMeasure) {
    ...
    if ((swing8CheckBox.checked && duration.denominator == 8) ||
        (swing16CheckBox.checked && duration.denominator == 16)) {
  ...
  function getOctaveModifier(prevNote, curNote) {
    ...
    if (prevOctave == curOctave) 
      return "";
     else if (curOctave == prevOctave + 1) 
      return ">";
     else if (curOctave == prevOctave - 1) 
      return "<";
     else 
      return "o" + curOctave + space;
  }
  ...
  function processAnnotations(texts, staff) {
    ...
    var prefix = "";
    var postfix = "";
    for (var i = 0; i < texts.length; i++) { // Make sure it's on the current staff
  ...
  function processScore() {
    ...
    if (swing8CheckBox.checked) {
      debugLog("Warning: Swing is turned on. Make sure that within each channel, in any beat where eighth notes occur, there are no notes of shorter" + " value. Otherwise, your channels may be out of sync. See the readme for more information.");
    }
    ...
    for (var staff = startStaff; staff <= endStaff; staff++) {
      ...
      while (cursor.segment) {
        ...
        if (cursor.measure.firstSegment.is(cursor.segment)) { // console.log("current: "+cursor.element.name);

          if (measure_spacesCheckBox.checked &&
              cursor.element.type !== Element.REPEAT_MEASURE &&
              prevType !== Element.REPEAT_MEASURE) {
            measure_segments += " ";
          }
        ...
        if (cursor.element && cursor.element.visible) { // check note visibility and color
          ...
          if (cursor.element.type === Element.CHORD) {
            if (cursor.element.notes.length > 1) {
              debugLog("Warning: Multiple notes found in chord at measure " + getMeasureNumber(cursor.measure) + ", channel " + staff + ". All but one will be ignored.");
            }
